...
import numpy as np
import torch
from model import ModularLSTM, ModularMLP
...
from utils.utils import set_seed, get_id_fields
from utils.enums import ModelType, Task
...
def prob_metrics(args: ModArgs, model):
    ...
    with torch.no_grad():
        for _ in range(1000):
            x, y, op = generate_data(args=args, n=1000)
    ...
    for model_dir in tqdm(SAVED_MODELS_DIR.iterdir()):
        ...
        with torch.no_grad():
            for eval_seed in range(100):
                ...
                for _ in range(10):
                    x, y, op = generate_data(args = args, n=1000, prob=p)
